  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100">
        <h3 className="font-bold text-gray-800">Ordens de Produção em Atraso</h3>
      </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 text-xs uppercase text-gray-500 font-semibold">
            <tr>
              ...
              <th className="px-6 py-3">Data Planejada</th>
              <th className="px-6 py-3">Data Real</th>
              <th className="px-6 py-3">Dias Atraso</th>
          ...
          <tbody className="divide-y divide-gray-100 text-sm">
            {data.map((item, i) => (
              <tr key={i} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-red-600 font-medium">{item.actual_date ?? 'Em aberto'}</td>
                <td className="px-6 py-4">
                  <span className="text-red-600 font-bold">{calculateDelay(item.planned_date, item.actual_date)} dias</span>
                </td>
                <td className="px-6 py-4">
            ...
            {data.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                  Nenhuma ordem atrasada detectada.
                </td>
              </tr>
            )}
          </tbody>
